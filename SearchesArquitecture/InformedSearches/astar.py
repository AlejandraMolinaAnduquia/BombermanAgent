...
import heapq
import math
from collections import Counter  # Import Counter for easy frequency tracking
...
class AStarSearch(SearchStrategy):
    ...
    def __init__(self, heuristic='Manhattan'):
        ...
        # NEW: Initialize a Counter to track frequency of tentative_g_score values
        self.g_score_frequencies = Counter()  
    ...
    def explore_step(self, agent, diagonal=False):
        ...
        if any(isinstance(a, GoalAgent) for a in agents_in_cell):
            ...
            # NEW: Print the most and least common tentative_g_score values when goal is reached
            most_common_cost = self.g_score_frequencies.most_common(1)[0]
            least_common_cost = min(self.g_score_frequencies.items(), key=lambda x: x[1])
            print(f"Most common cost: {most_common_cost[0]} (repeated {most_common_cost[1]} times)")
            print(f"Least common cost: {least_common_cost[0]} (repeated {least_common_cost[1]} times)")
        ...
        for direction in directions:
            ...
            if (
                ...
                if all(isinstance(agent, (RoadAgent, GoalAgent, RockAgent, GlobeAgent)) for agent in agents_in_new_cell):
                    ...
                    # NEW: Track the frequency of each tentative_g_score
                    self.g_score_frequencies[tentative_g_score] += 1
